...
export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ bitId: string }> }
) {
  try {
    const resolvedParams = await params;
    const bitId = resolvedParams.bitId;
    ...
    console.log('🔍 DEBUG: Bits like API route called for bitId:', bitId);
    console.log('🔍 DEBUG: Token exists:', !!token);

    const response = await fetch(`https://www.trendshub.link/api/v1/bits/${bitId}/like`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: data.message || data.error || 'Failed to like bit' }, { status: response.status });
    }

    console.log('🔍 DEBUG: Bits like API route success, returning data');
    return NextResponse.json(data);
  } catch (error) {
    console.error('🔍 DEBUG: Unhandled error in bits like API route:', error);

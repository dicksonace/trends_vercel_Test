...
export async function GET(
  ...
  try {
    ...
    console.log('🔍 DEBUG: Like status API route called for postId:', postId);
    console.log('🔍 DEBUG: Token exists:', !!token);

    const response = await fetch(`https://www.trendshub.link/api/v1/likes/status/${postId}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: data.message || data.error || 'Failed to get like status' }, { status: response.status });
    }

    console.log('🔍 DEBUG: Like status API route success, returning data');
    return NextResponse.json(data);
  } catch (error) {
    console.error('🔍 DEBUG: Unhandled error in like status API route:', error);

...
export async function GET(
  ...
  try {
    ...
    console.log('🔍 DEBUG: Bookmark status API route called for postId:', postId);
    console.log('🔍 DEBUG: Token exists:', !!token);

    const response = await fetch(`https://www.trendshub.link/api/v1/bookmark-status/${postId}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: data.message || data.error || 'Failed to get bookmark status' }, { status: response.status });
    }

    console.log('🔍 DEBUG: Bookmark status API route success, returning data');
    return NextResponse.json(data);
  } catch (error) {
    console.error('🔍 DEBUG: Unhandled error in bookmark status API route:', error);

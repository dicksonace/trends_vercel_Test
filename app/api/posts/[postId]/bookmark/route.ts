...
export async function POST(
  ...
  try {
    ...
    console.log('🔍 DEBUG: Bookmark API route called for postId:', postId);
    console.log('🔍 DEBUG: Token exists:', !!token);

    const response = await fetch(`https://www.trendshub.link/api/v1/bookmark/${postId}`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: data.message || data.error || 'Failed to toggle bookmark' }, { status: response.status });
    }

    console.log('🔍 DEBUG: Bookmark API route success, returning data');
    return NextResponse.json(data);
  } catch (error) {
    console.error('🔍 DEBUG: Unhandled error in bookmark API route:', error);
